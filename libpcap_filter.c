...
void sigint_handler(int sig_no)//, int number_of_bins, struct bin bins[])
{
...
int main(int argc, char *argv[]) {
    ...
    while (1){
        ...
        if (packet != NULL){
            ...
            if (strcmp(version, "6") == 0){
                if (ntohs(eth_h->h_proto) == ETH_P_IPV6) {
                    ...
                    //printf("time: %ld, Flow label for this packet: 0x%x\n", begin.tv_sec, ipv6_fl);
        ...
        // if((end.tv_sec - begin.tv_sec) >= DELTAT)
        // {
